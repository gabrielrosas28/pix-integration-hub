...
public interface IBankPixAdapter
{
    ...
    /// </summary>
    Task<ChargeResponse> CreateCobAsync(ChargeRequest request, CancellationToken cancellationToken = default);

    /// <summary>
    /// Cria uma cobrança Pix com vencimento (CobV).
    /// </summary>
    Task<ChargeResponse> CreateCobVAsync(ChargeRequest request, CancellationToken cancellationToken = default);

    /// <summary>
    /// Obtém o QR Code (EMV + Pix Link) de uma cobrança.
    /// </summary>
    Task<QrCodeResponse> GetQrCodeAsync(string txId, PixChargeType type, CancellationToken cancellationToken = default);

    /// <summary>
    /// Consulta o status atual de uma cobrança.
    /// Este é o método que valida definitivamente se um Pix foi pago.
    /// </summary>
    Task<ChargeStatusResponse> GetChargeStatusAsync(string txId, PixChargeType type, CancellationToken cancellationToken = default);
